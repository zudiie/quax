...
public class Main extends ApplicationAdapter {
    ...
    public void render() {
        ...
        if (showWelcome) {
            ...
            float titleX = boardCenterX - wlTitle.width / 2f;
            float titleY = boardCenterY + 120f;
            font.draw(batch, welcomeTitle, titleX, titleY);

            String subTitle = "Human vs Human";
            GlyphLayout wlSub = new GlyphLayout(font, subTitle);
            float subX = boardCenterX - wlSub.width / 2f;
            float subY = boardCenterY + 80f;
            font.draw(batch, subTitle, subX, subY);

            // Draw button text on top of the button shape
            ...
            if (startButtonBounds != null) {
                ...
                shapeRenderer.setColor(hover ? 0.25f : 0.2f, 0.2f, 0.2f, 0.9f);
        ...
        if (!statusMessage.isEmpty()) {
            ...
            float statusY = viewport.getWorldHeight() - 20f; // near top
    ...
    private void handleInput() {
        ...
        for (MapObject object : diamondLayer.getObjects()) {
            if (object instanceof TextureMapObject) {
                ...
                if (touchPos.x >= worldX && touchPos.x <= worldX + objW &&
                    ...
                    // occupied diamond: show feedback, do not switch turn
        ...
        if (cell != null && cell.getTile() != null) {
            ...
            // if the cell is already occupied by any stone, show feedback and do not change turn
    ...
    private void computeButtonBounds() {
        ...
        // Start button centered on the screen for the welcome view
        float startW = 220;
        float startH = 60;
        float startX = boardCenterX - startW / 2f;
        float startY = boardCenterY - 10f;
